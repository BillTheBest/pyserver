...
class Bencode(Encoding):
    ...
    def _encode(self, data):
        ...
        if type(data) in (int, long):
            ...
            encodedListItems = ''
            for item in data:
                encodedListItems += self._encode(item)
            ...
            keys.sort()
            for key in keys:
                encodedDictItems += self._encode(key)
                encodedDictItems += self._encode(data[key])
...
class GPGBencode(Bencode):
    """ Implementation of a Bencode-based algorithm (Bencode is the encoding
    algorithm used by Bittorrent), verifying data using GPG.
    """
